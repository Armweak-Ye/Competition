...
def detect_rescue(target_shape):
    ...
    for _ in range(3):
        ...
        img.draw_cross(roi_cx, roi_cy, color=127)

        centered = (abs(b.cx() - roi_cx) <= TOLERANCE) and \
                   (abs(b.cy() - roi_cy) <= TOLERANCE)

        if not centered:
            img.draw_string(b.x() + 2, b.y() - 14,
                            "X=%d Y=%d" % (b.cx(), b.cy()), color=255)
            img.draw_string(0, 0, "FPS:%.1f" % sensor.get_fps())
            return (_quant_x(b.cx()), _quant_y(b.cy()), 0)
        ...
        if r:
            ...
            info = "MATCH X=%d Y=%d" % (r.x(), r.y())
            img.draw_string(r.x() + 2, r.y() - 14, info, color=255)
            img.draw_string(0, 0, "FPS:%.1f" % sensor.get_fps())
            raw_x = r.x() + _template.width() // 2
            raw_y = r.y() + _template.height() // 2
            X = _quant_x(raw_x)
            Y = _quant_y(raw_y)
        ...
        img.draw_string(b.x() + 2, b.y() - 14,
                        "X=%d Y=%d NO" % (b.cx(), b.cy()), color=255)
        img.draw_string(0, 0, "FPS:%.1f" % sensor.get_fps())
        return (_quant_x(b.cx()), _quant_y(b.cy()), 0)
